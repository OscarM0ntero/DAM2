	...
	getPeliculasFav(idUsuario: number): Observable<Pelicula[]> {
		return this.getFav(idUsuario).pipe(
			switchMap((ids: number[]) => {
				if (!ids.length) return of([]); // ✅ Si no hay IDs, devuelve un array vacío

				// ✅ Crear un array de peticiones usando forkJoin
				const requests = ids.map(id =>
					this.getPeliculaById(id.toString()).pipe(
						catchError(() => of(undefined)) // ✅ Si alguna petición falla, devuelve undefined
					)
				);

				// ✅ Ejecutar todas las peticiones de forma simultánea
				return forkJoin(requests).pipe(
					map((peliculas) =>
						peliculas.filter((pelicula): pelicula is Pelicula => !!pelicula) // ✅ Filtrar las que sean undefined
					)
				);
			})
		);
